...
void BeliefManager<PomdpType, BeliefValueType, StateType>::adjustDistribution(DistributionType &distr) {
    if (distr.size() == 1 && cc.isEqual(distr.begin()->second, storm::numbers::one<BeliefValueType>())) {
        ...
        distr.begin()->second = storm::numbers::one<BeliefValueType>();
    }
}
...
bool BeliefManager<PomdpType, BeliefValueType, StateType>::assertBelief(BeliefType const &belief) const {
    auto sum = storm::numbers::zero<BeliefValueType>();
    ...
    for (auto const &entry : belief) {
        ...
        if (storm::numbers::isZero(entry.second)) {
            // We assume that beliefs only consider their support.
            STORM_LOG_ERROR("Zero belief probability.");
            return false;
        }
        if (entry.second < storm::numbers::zero<BeliefValueType>()) {
            STORM_LOG_ERROR("Negative belief probability.");
            return false;
        }
        if (cc.isLess(storm::numbers::one<BeliefValueType>(), entry.second)) {
...
bool BeliefManager<PomdpType, BeliefValueType, StateType>::assertTriangulation(BeliefType const &belief, Triangulation const &triangulation) const {
    ...
    auto weightSum = storm::numbers::zero<BeliefValueType>();
    for (uint64_t i = 0; i < triangulation.weights.size(); ++i) {
        ...
        if (cc.isLess(triangulation.weights[i], storm::numbers::zero<BeliefValueType>())) {
            STORM_LOG_ERROR("Negative weight in triangulation.");
            return false;
        }
        if (cc.isLess(storm::numbers::one<BeliefValueType>(), triangulation.weights[i])) {
        ...
        for (auto const &pointEntry : gridPoint) {
            BeliefValueType &triangulatedValue = triangulatedBelief.emplace(pointEntry.first, storm::numbers::zero<BeliefValueType>()).first->second;
...
void BeliefManager<PomdpType, BeliefValueType, StateType>::triangulateBeliefFreudenthal(BeliefType const &belief, BeliefValueType const &resolution,
    ...
    STORM_LOG_ASSERT(storm::numbers::isInteger(resolution), "Expected an integer resolution.");
    ...
    for (auto const &entry : belief) {
        qsRow.push_back(storm::numbers::floor(x));                            // v
    ...
    qsRow.push_back(storm::numbers::zero<BeliefValueType>());
    ...
    for (StateType i = 0; i < numEntries; ++i) {
        ...
        if (i == 0) {
            ...
            weight += storm::numbers::one<BeliefValueType>();
        } else {
            // 'compute' the next row of the qs matrix
            qsRow[previousSortedDiff->dimension] += storm::numbers::one<BeliefValueType>();
        }
        if (!cc.isZero(weight)) {
...
void BeliefManager<PomdpType, BeliefValueType, StateType>::triangulateBeliefDynamic(BeliefType const &belief, BeliefValueType const &resolution,
    ...
    for (BeliefValueType currResolution = resolution; currResolution > resolution / 2; --currResolution) {
        ...
        for (auto const &belEntry : belief) {
            ...
            if (!cc.isZero(product - storm::numbers::round(product))) {
                ++currResMisses;
...
typename BeliefManager<PomdpType, BeliefValueType, StateType>::Triangulation BeliefManager<PomdpType, BeliefValueType, StateType>::triangulateBelief(
    ...
    if (belief.size() == 1u) {
        result.weights.push_back(storm::numbers::one<BeliefValueType>());
        result.gridPoints.push_back(getOrAddBeliefId(belief));
    } else {
        auto ceiledResolution = storm::numbers::ceil<BeliefValueType>(resolution);
...
typename BeliefManager<PomdpType, BeliefValueType, StateType>::BeliefClipping BeliefManager<PomdpType, BeliefValueType, StateType>::clipBeliefToGrid(
    ...
    auto bigDelta = lpSolver->addBoundedContinuousVariable("D", storm::numbers::zero<BeliefValueType>(), storm::numbers::one<BeliefValueType>(),
                                                           storm::numbers::one<BeliefValueType>());
    ...
    for (auto const &state : belief) {
        ...
        if (typeid(*lpSolver) == typeid(storm::solver::GlpkLpSolver<ValueType>) && !isInfinite.empty()) {
            if (isInfinite[state.first]) {
                auto localDelta = lpSolver->addBoundedContinuousVariable("d_" + std::to_string(i), storm::numbers::zero<BeliefValueType>(), state.second);
                auto deltaExpr = storm::expressions::Expression(localDelta);
                deltas.push_back(deltaExpr);
                lpSolver->addConstraint("state_val_inf_" + std::to_string(i), deltaExpr == lpSolver->getConstant(storm::numbers::zero<BeliefValueType>()));
            }
        } else {
            BeliefValueType bound = state.second;
            if (!isInfinite.empty()) {
                bound = isInfinite[state.first] ? storm::numbers::zero<BeliefValueType>() : state.second;
            }
            auto localDelta = lpSolver->addBoundedContinuousVariable("d_" + std::to_string(i), storm::numbers::zero<BeliefValueType>(), bound);
    ...
    while (!done) {
        ...
        if (isEqual(candidate, belief)) {
            ...
            return BeliefClipping{false, noId(), noId(), storm::numbers::zero<BeliefValueType>(), {}, true};
        } else {
            ...
            for (auto const &state : belief) {
                ...
                if (candidate.find(state.first) != candidate.end()) {
                    ...
                    targetValue = lpSolver->getConstant(storm::numbers::zero<BeliefValueType>());
                }
                ...
                storm::expressions::Expression rightSide =
                    ...
                    lpSolver->getConstant(storm::numbers::one<BeliefValueType>()) + storm::expressions::Expression(decisionVar);
    ...
    lpSolver->addConstraint("choice", storm::expressions::sum(decisionVariables) == lpSolver->getConstant(storm::numbers::one<BeliefValueType>()));
    // Link D and d_i
    lpSolver->addConstraint("delta", storm::expressions::Expression(bigDelta) == storm::expressions::sum(deltas));
    // Exclude D = 0 (self-loop)
    lpSolver->addConstraint("not_zero", storm::expressions::Expression(bigDelta) > lpSolver->getConstant(storm::numbers::zero<BeliefValueType>()));
    ...
    auto optDelta = storm::numbers::zero<BeliefValueType>();
    auto deltaSum = storm::numbers::zero<BeliefValueType>();
    if (lpSolver->isOptimal()) {
        ...
        for (auto const &state : belief) {
            ...
            if (cc.isLess(storm::numbers::zero<BeliefValueType>(), val)) {
        ...
        if (cc.isEqual(optDelta, storm::numbers::zero<BeliefValueType>())) {
            ...
            return BeliefClipping{false, noId(), noId(), storm::numbers::zero<BeliefValueType>(), {}, false};
        }

        if (optDelta == storm::numbers::one<BeliefValueType>()) {
            ...
            if (deltaSum == storm::numbers::one<BeliefValueType>()) {
                return BeliefClipping{false, noId(), noId(), storm::numbers::zero<BeliefValueType>(), {}, false};
            }
            optDelta = deltaSum;
        }
...
typename BeliefManager<PomdpType, BeliefValueType, StateType>::BeliefId BeliefManager<PomdpType, BeliefValueType, StateType>::computeInitialBelief() {
    ...
    BeliefType belief;
    belief[*pomdp.getInitialStates().begin()] = storm::numbers::one<BeliefValueType>();
...
std::vector<BeliefValueType> BeliefManager<PomdpType, BeliefValueType, StateType>::getBeliefAsVector(const BeliefType &belief) {
    std::vector<BeliefValueType> res(pomdp.getNumberOfStates(), storm::numbers::zero<BeliefValueType>());
