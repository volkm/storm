...
struct BeliefExplorationPomdpModelCheckerOptions {
    ...
    ValueType refinePrecision = storm::numbers::convertNumber<ValueType>(1e-4);
    uint64_t explorationTimeLimit = 0;

    // Control parameters for the refinement heuristic
    // Discretization Resolution
    uint64_t resolutionInit = 2;
    ValueType resolutionFactor = storm::numbers::convertNumber<ValueType, uint64_t>(2);
    // The maximal number of newly expanded MDP states in a refinement step
    uint64_t sizeThresholdInit = 0;
    ValueType sizeThresholdFactor = storm::numbers::convertNumber<ValueType, uint64_t>(4);
    // Controls how large the gap between known lower- and upper bounds at a belief state needs to be in order to explore
    ValueType gapThresholdInit = storm::numbers::convertNumber<ValueType>(0.1);
    ValueType gapThresholdFactor = storm::numbers::convertNumber<ValueType>(0.25);
    // Controls whether "almost optimal" choices will be considered optimal
    ValueType optimalChoiceValueThresholdInit = storm::numbers::convertNumber<ValueType>(1e-3);
    ValueType optimalChoiceValueThresholdFactor = storm::numbers::one<ValueType>();
    // Controls which observations are refined.
    ValueType obsThresholdInit = storm::numbers::convertNumber<ValueType>(0.1);
    ValueType obsThresholdIncrementFactor = storm::numbers::convertNumber<ValueType>(0.1);
    ...
    ValueType numericPrecision = storm::numbers::NumberTraits<ValueType>::IsExact
                                     ...
                                     : storm::numbers::convertNumber<ValueType>(1e-9);  /// Used to decide whether two beliefs are equal
