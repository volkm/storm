...
std::unique_ptr<CheckResult> SparseDtmcInstantiationModelChecker<SparseModelType, ConstantType>::check(
    ...
    if (instantiatedModel.isExact()) {
        std::string reasonNotProbabilistic;
        STORM_LOG_THROW(instantiatedModel.getTransitionMatrix().isProbabilistic(storm::utility::zero<ConstantType>(), reasonNotProbabilistic),
                        storm::exceptions::InvalidArgumentException,
                        "Instantiation point is invalid as the transition matrix becomes non-stochastic. " << reasonNotProbabilistic);
    } else {
