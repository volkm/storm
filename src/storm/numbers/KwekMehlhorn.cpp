...
std::pair<IntegerType, IntegerType> findRational(IntegerType const& alpha, IntegerType const& beta, IntegerType const& gamma, IntegerType const& delta) {
    std::pair<IntegerType, IntegerType> alphaDivBetaPair = storm::numbers::divide(alpha, beta);
    std::pair<IntegerType, IntegerType> gammaDivDeltaPair = storm::numbers::divide(gamma, delta);

    if (alphaDivBetaPair.first == gammaDivDeltaPair.first && !storm::numbers::isZero(alphaDivBetaPair.second)) {
        ...
        std::pair<IntegerType, IntegerType> result(
            storm::numbers::isZero(alphaDivBetaPair.second) ? alphaDivBetaPair.first : alphaDivBetaPair.first + storm::numbers::one<IntegerType>(),
            storm::numbers::one<IntegerType>());
        return result;
    }
}
